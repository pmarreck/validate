...
- [ ] **Memory subsystem (raised 2026-04-28; two concurrent runs OOM'd 128 GB Mac):**
  ...
  - [ ] **Racetrack allocator experiment (codec-internal only)** — Peter's "fair racetrack" design: fixed ring with sliding 1/2-window of in-use blocks, bump-allocated, no defrag. Convoy hazard if mixed-lifetime; *therefore* prove it inside JPEG/PNG/FLAC streaming validators where lifetimes are uniform and bounded. Per-class racetracks (filetype-grouped) deferred until single-ring proves out — multi-class mitigates convoy by lifetime homogeneity per class.
