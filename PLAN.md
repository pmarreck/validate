...
- [ ] **Memory subsystem (raised 2026-04-28; two concurrent runs OOM'd 128 GB Mac):**
  - [ ] **Per-task arena allocator** — replace per-validator allocator pass-through with `ArenaAllocator` per file; `deinit()` reclaims wholesale, eliminates cross-task fragmentation, contains C-library leaks. Foundation for everything below.
  - [ ] **`page_allocator` sweep** — 127 sites across ~15 files. Two patterns: (a) ~25 fixed-64KB scratch buffers → stack-allocate; (b) ~100 file-size-dependent → thread allocator parameter. Worst-offender files: text_format_validators (21), music_validators (15), format_validation (12), archive_validators (12), scientific_validators (10).
  - [ ] **Library threading lockdown** in `cli/main.c` — `OMP_NUM_THREADS=1`, `OPJ_NUM_THREADS=1`, equivalent for libheif/libavif/libdav1d. 5-line change; eliminates uncontrolled internal threading spikes.
  - [ ] **Budget-gated work queue** — `--memory-budget=N{K,M,G}` flag and `VALIDATE_MEMORY_BUDGET` env var, default `clamp(ram/3, 1GB, 8GB)`. Workers wait for budget before pulling next task; oversized tasks admitted alone (starvation rule).
  - [ ] **Streaming codec refactor** — biggest single win. Most validators currently slurp full file into memory before passing to a codec that already supports streaming. Targets in priority order: JPEG (libjpeg-turbo `jpeg_stdio_src`), PNG (libpng `set_progressive_read_fn`), FLAC (per-frame discard callback), zlib/gzip/bzip2/zstd (loop-pump), Vorbis/Opus (`ov_read`). Goal: O(input_window + decoder_state) regardless of file size.
  - [ ] **Big-allocation diversion** — any single allocation past some fraction of the budget goes to `mmap` instead of through the managed allocator. Removes giants from the budget pool.
  - [x] **Racetrack allocator experiment** — `src/core/racetrack.zig` shipped 2026-04-29. Fixed-buffer bump allocator with sliding-window invariant, FIFO live-block tracking, wraparound via skip-pad. Load test directly compares against per-task arena under two workloads. **Heterogeneous (10% large 1-4 MB / 90% small 1-8 KB, 10K ops, 8 workers, 16 MB window):** racetrack 108ms / 9.2K ops/sec / 7.3% blocked / 11.4ms max stall vs arena 81ms / 12.4K ops/sec — **arena wins 1.34×**, convoy confirmed. **Uniform (all small):** racetrack 6.5ms / 154K ops/sec / 0% blocked vs arena 15.4ms / 65K ops/sec — **racetrack wins 2.4×**. Conclusion: deploy racetrack ONLY inside codec-internal contexts where lifetimes are uniform; per-task arena stays the outer-scheduler choice. Per-class racetracks (filetype-grouped) deferred indefinitely — single-ring proves out enough.
  - [x] **Profiling harness** — `scripts/profile-memory` landed 2026-04-28. Spawns N concurrent `validate` runs, samples RSS+VSZ via `ps` every 100ms, reports per-pid peak/avg + aggregate p50/p95/peak.
