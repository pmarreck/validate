...
**HARD CONSTRAINT — NO UNDECLARED CODEGEN INTERPRETER.** LibRaw's default build pulls in an interpreter for some build tooling. We do **not** adopt that dependency under any circumstances. Find LibRaw's build path that avoids it; **fork LibRaw if necessary** to strip the build dependency (use `find_github_forks_with_file` to look for an existing fork with a clean `flake.nix`/`build.zig`/`CMakeLists` that already builds without it; check Peter's sibling dirs first per memory). Document the build recipe.

**This Phase warrants its own dedicated plan** (build wiring + cross-target CI is substantial). Outline of the work:

- [ ] **Step 1 — Interpreter-free LibRaw build:** Establish a LibRaw build (vendored fork or patched derivation) with the undeclared build-time dependency removed. Add to `flake.nix`. Verify it builds in Nix's sandbox (no network) per the project's Zig+Nix+Garnix dep strategy.
