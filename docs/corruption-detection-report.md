...
| APE (Monkey's Audio) | 3% | 5% | corpus_synthetic.ape | 16 KB | 2026-04-26 | MAC header + descriptor MD5 + per-frame CRC32. Validator now performs structural-rigor parsing (descriptor, header, seek-table monotonicity, audio-region bounds, sample-rate/channels/BPS sanity, version range). The descriptor MD5 and per-frame CRC32 fields cover **decoded PCM samples** so verifying them requires a full Monkey's Audio decoder (range coder + IIR predictor + entropy + 4-mode filter pipeline) — deferred to a multi-session task (~2000 LOC port of FFmpeg `apedec.c` or vendor with extensive shim). Synthetic 16 KB corpus has 80 frames so ~2.4% of file is validated structure (descriptor + header + seek table); sniper/shotgun rates reflect that surface area. See Action Items #13. |
...
| WavPack | **100%** | **100%** | corpus_xorshift.wv | 118 KB | 2026-04-26 | libwavpack 5.9.0 deep decode: every block decoded to PCM, per-block CRC over decoded samples + checksum sub-block + sample-count drift. Block-checksum sub-block (`ID_BLOCK_CHECKSUM`) catches header/bitstream tampering at open-time; post-decode CRC catches arithmetic drift; truncation surfaces as decoded < expected sample count. |
...
13. **APE MD5 still pending — WavPack DONE 2026-04-26.** WavPack now achieves **100%/100%** via libwavpack 5.9.0 deep-decode integration (commit follows this dispatch). The CRC field in each WavPack block header (computed over decoded PCM per dbry's `unpack.c` line 508) is now verified by decoding every block and querying `WavpackGetNumErrors`. Block-checksum sub-blocks catch header/bitstream tampering at open-time; sample-count drift catches truncation. APE remains at structural-only because its 32-bit per-frame CRC is also post-decode (FFmpeg's `apedec.c` confirms iteration over `frame->data[ch]` after entropy decode + predictor). APE deep validation requires a separate decoder integration (~5-7 sessions or vendoring `mac` / `ffmpeg` libavcodec subset).
