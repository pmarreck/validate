...
if parsed then
	...
	ok(parsed.available[2].key == "linux-x86_64", "Linux x86_64 follows macOS")
	ok(parsed.available[3].key == "linux-aarch64", "Linux ARM64 stays adjacent to Linux x86_64")
	ok(parsed.available[4].key == "windows-x86_64", "Windows x86_64 begins the grouped Windows row")
	ok(parsed.available[5].key == "windows-aarch64", "Windows ARM64 completes the grouped Windows row")
