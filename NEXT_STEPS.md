...
| 4g. CABAC bit under-consumption — RESIDUAL | (same) | **STILL OPEN.** Each spec-correct fix in 4b-4f exposed another compensating bug, so per-file CTU counts shifted unpredictably between fixes. Net trajectory is clearly upward (e.g., `summer_1440x960.heic` went `27 -> 149 -> 75 -> 345` CTUs across the session); but `crowd` (`345 -> 26 -> 303 -> 11`) and others swing. Strong evidence the *engine* now responds to corruption (autumn corruption at file bytes 5000/8000/15000 produces ctus_decoded `143/104/345` vs clean's `162` — clearly different signatures). Corruption past the under-decode boundary (file byte ~28K for autumn) is still invisible. Next likely culprits: (1) non-4x4 sub-block `sig_coeff_flag` context derivation in `getSigCoeffCtxSpec` (the spec uses prev_csbf-keyed lookup tables, not the closed-form approximation in the current code); (2) `split_cu_flag` ctxInc — currently uses depth-only approximation; spec requires neighbor depth lookup; (3) IntraSplitFlag exclusion in `transformTree` (split_transform_flag should NOT be coded at trafoDepth=0 for NxN intra CUs). |
