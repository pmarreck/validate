...
| `src/core/document_validators.zig` | Document/database format validation — SQLite (page size + B-tree walk), OLE2 dispatch (DOC/XLS/PPT/MSI/Thumbs.db via Catalog stream), WordPerfect, MDB/ACCDB (Access), dBASE .dbf (version byte + field descriptor cross-validation) |
